...
const MediumButton = ({ onPress, children, btnStyle, labelStyle, shadow, icon, iconStyle, loading, disable }) => {
    const { buttonContainer, txtStyle, shadowStyle } = styles
    return (
        <RippleEffect disabled={loading || disable} style={[buttonContainer,{backgroundColor:disable? '#aaa':mainBtnBg}, btnStyle]} onPress={onPress} >
            {shadow ?
                <ShadowView
